...
export class GameUI {
	...
	showRecords(result, onBack) {
		...
		const renderPage = (pageIndex) => {
			...
			const navigation = leaderboards.length > 1 ? `<p style="margin:18px 0 0;display:flex;gap:8px;justify-content:center"><button data-action="previous-rank" type="button" aria-label="Previous rank" title="Previous rank" ${pageIndex === 0 ? 'disabled' : ''} style="width:48px;height:40px;border:1px solid ${PALETTE.uiText};background:#ffffff;color:${PALETTE.uiText};font-family:Georgia, 'Times New Roman', serif;font-size:28px;line-height:1">‹</button><button data-action="next-rank" type="button" aria-label="Next rank" title="Next rank" ${pageIndex === leaderboards.length - 1 ? 'disabled' : ''} style="width:48px;height:40px;border:1px solid ${PALETTE.uiText};background:#ffffff;color:${PALETTE.uiText};font-family:Georgia, 'Times New Roman', serif;font-size:28px;line-height:1">›</button></p>` : '';
			this.root.innerHTML = `<div style="height:100%;display:grid;place-items:center;pointer-events:auto"><section style="box-sizing:border-box;min-width:280px;max-width:320px;padding:24px;background:#ffffff;border:1px solid ${PALETTE.uiText};text-align:center"><h2 style="margin:0;color:${PALETTE.uiAccent};font-size:30px">records</h2>${content}${navigation}<p style="margin:18px 0 0"><button data-action="back" style="${secondaryButtonStyle}">back</button></p></section></div>`;
