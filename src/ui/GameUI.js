...
export class GameUI {
	...
	showRecords(result, onBack) {
		...
		const renderPage = (pageIndex) => {
			...
			this.root.innerHTML = `<div style="height:100%;display:grid;place-items:center;pointer-events:auto"><section style="box-sizing:border-box;width:calc(100vw - 32px - env(safe-area-inset-left) - env(safe-area-inset-right));height:406px;max-width:394px;padding:24px;background:#ffffff;border:1px solid ${PALETTE.uiText};text-align:center;display:flex;flex-direction:column"><h2 style="margin:0;color:${PALETTE.uiAccent};font-size:30px">records</h2>${content}<div style="margin-top:auto">${navigation}<p style="margin:18px 0 0"><button data-action="back" style="${secondaryButtonStyle}">back</button></p></div></section></div>`;
