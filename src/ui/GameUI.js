...
export class GameUI {
	...
	showRecords(result, onBack) {
		...
		const renderPage = (pageIndex) => {
			...
			this.root.innerHTML = `<div style="height:100%;display:grid;place-items:center;pointer-events:auto"><section style="box-sizing:border-box;width:394px;height:406px;max-width:100%;padding:24px;background:#ffffff;border:1px solid ${PALETTE.uiText};text-align:center"><h2 style="margin:0;color:${PALETTE.uiAccent};font-size:30px">records</h2>${content}${navigation}<p style="margin:18px 0 0"><button data-action="back" style="${secondaryButtonStyle}">back</button></p></section></div>`;
